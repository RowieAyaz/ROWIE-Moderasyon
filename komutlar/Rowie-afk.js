...
exports.run = async (client, message, args) => {
  ...
  if (!args[0]) {
    ...
    const rowie = await db.fetch(
      `afkSebep_${message.author.id}_${message.guild.id}`
    );

   message.channel.send(new MessageEmbed().setColor('BLACK').setDescription(`${kullanıcı} Başarıyla Afk Oldunuz Afk Olmanızın Sebebi: **${rowie}**`));
